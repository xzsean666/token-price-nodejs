...
export function classifyBinanceResponse(response: HttpResponse): TokenPriceError | null {
...
export function normalizeBinanceTransportError(value: unknown): TokenPriceError | null {
...
function error(code: TokenPriceError["code"], message: string, retryable: boolean, retryAfterMs?: number): TokenPriceError {
  return new TokenPriceError({ code, message, retryable, provider: "binance", ...(retryAfterMs === undefined ? {} : { retryAfterMs }) });
}
